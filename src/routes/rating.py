...
from schemas import RatingResponseModel, RetingRequestModel
...
@has_role("user")
async def create_rating(body: RetingRequestModel, image_id: int, db: Session = Depends(get_db), current_user: User = Depends(auth_service.get_current_user)):
